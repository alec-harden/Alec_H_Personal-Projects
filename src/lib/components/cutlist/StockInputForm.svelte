<script lang="ts">
	import type { Stock, CutListMode } from '$lib/types/cutlist';
	import { createStock } from '$lib/types/cutlist';
	...
	let { stock = $bindable(), mode }: Props = $props();

	function addStock() {
		stock = [...stock, createStock(mode)];
	}
...
<section class="input-section">
	<div class="section-header">
		<h3 class="section-title">Available Stock</h3>
		<button type="button" onclick={addStock} class="btn-secondary btn-sm">
	...
	{#if stock.length === 0}
		<div class="empty-state">
			<p class="empty-message">Add at least one stock piece to begin optimization.</p>
		</div>
	{:else}
			...
			{#each stock as stockItem (stockItem.id)}
				<div class="entry-row">
					...
					<div class="entry-field entry-actions">
						<button
							type="button"
							onclick={() => removeStock(stockItem.id)}
